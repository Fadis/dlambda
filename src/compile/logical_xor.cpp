...
#include <dlambda/compiler/node/logical_xor.hpp>
...
namespace dlambda {
  namespace compiler {
    namespace node {
      class logical_xor : public boost::static_visitor< expression > {
      public:
        logical_xor(
    ...
    expression logical_xor(
      ...
      return apply_visitor( node::logical_xor( context, ir_builder, left, right ), left.type(), right.type() );
    }
  }
}
