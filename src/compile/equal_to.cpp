...
namespace dlambda {
  namespace compiler {
    namespace node {
      class equal_to : public boost::static_visitor< expression > {
      public:
        equal_to(
    ...
    expression equal_to(
      ...
      return apply_visitor( node::equal_to( context, ir_builder, left, right ), left.type(), right.type() );
    }
  }
}
