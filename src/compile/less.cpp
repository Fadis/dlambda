...
namespace dlambda {
  namespace compiler {
    namespace node {
      class less : public boost::static_visitor< expression > {
      public:
        less(
    ...
    expression less(
      ...
      return apply_visitor( node::less( context, ir_builder, left, right ), left.type(), right.type() );
    }
  }
}
