...
namespace dlambda {
  namespace compiler {
    namespace node {
      class less_equal : public boost::static_visitor< expression > {
      public:
        less_equal(
      ...
      expression less_equal::generate(
        const expression &left,
        const expression &right
      ) const {
    ...
    expression less_equal(
      ...
      return apply_visitor( node::less_equal( context, ir_builder, left, right ), left.type(), right.type() );
    }
  }
}
